...
class Config:
    ...
    def get_data_set(self):
        ...
        reference_data = data_sets.inf_train_gen(data_config['reference'],
                                         batch_size=data_config['total_data'],
                                         dim=self.config['model']['input_dim'])
        # Generate target data
        target_data = data_sets.inf_train_gen(data_config['target'],
                                         batch_size=data_config['total_data'],
                                         dim=self.config['model']['input_dim'])
        # Convert to PyTorch dataset and create dataloader
        reference_data_set = TensorDataset(torch.tensor(reference_data).float().to(self.config['training']['device']))
        target_data_set = TensorDataset(torch.tensor(target_data).float().to(self.config['training']['device']))
        reference_data_loader = DataLoader(reference_data_set,batch_size=self.config['training']['batch_size'],shuffle=True)
        target_data_loader = DataLoader(target_data_set,batch_size=self.config['training']['batch_size'],shuffle=True)
        return reference_data_loader,target_data_loader
...
def loss_flow_matching(
        model: nn.Module,
        reference_data_set: torch.Tensor,
        targer_data_set: torch.Tensor,
        device: torch.device,
        batch_size=1000,
        dim=2):
    '''
    ...
    zt = targer_data_set*t[:,None] + (1-t[:,None])*reference_data_set  # Linear interpolation
    xt = torch.randn(batch_size,dim).to(device)*0 + zt  # Add noise
    
    # Compute target velocity (straight line to target)
    ut = targer_data_set-reference_data_set
...
def train_flow_matching(config_path):
    ...
    model = cfg.get_model()
    reference_data_loader,target_data_loader = cfg.get_data_set()
    optimizer = cfg.get_optimizer(model)
    device = cfg.config['training']['device']

    # Setup checkpoint directory
    path = cfg.config['training']['checkpoint_dir']+cfg.config['data']['reference']+'_'+cfg.config['data']['target']+\
    ...
    for epoch in prange:
        ...
        # Batch training
        for (reference_data,target_data) in zip(reference_data_loader,target_data_loader):
            optimizer.zero_grad()
            loss = loss_flow_matching(model, 
                                      reference_data_set=reference_data[0],
                                      targer_data_set=target_data[0], 
                                      device=device,
...
if __name__ == '__main__':    
    train_flow_matching(str(project_root)+'/configs/fm_training/configs_train_fm2.yaml')
